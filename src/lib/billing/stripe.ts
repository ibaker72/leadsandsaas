...
export async function createPortalSession(orgId: string): Promise<Result<{ url: string }>> {
  return tryCatch(async () => {
    ...
    const session = await getStripe().billingPortal.sessions.create({ customer: org.stripe_customer_id as string, return_url: `${process.env.NEXT_PUBLIC_APP_URL}/billing` });
    return { url: session.url };
  });
}
