...
import { Zap, AlertCircle, Loader2, Check, ArrowLeft } from 'lucide-react';
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(email.trim(), {
        // CRITICAL: redirect to callback route which exchanges the code,
        // then redirects to update-password page
          ...
          {sent ? (
            <div className="text-center">
              ...
              <p className="text-[14px] mb-6 leading-relaxed" style={{ color: 'var(--text-dark-secondary)' }}>
                If an account exists for <strong style={{ color: 'var(--text-dark)' }}>{email}</strong>, we sent a password reset link.
              </p>
              <Link href="/login" className="text-[13px] font-semibold hover:underline flex items-center justify-center gap-1.5" style={{ color: 'var(--accent)' }}>
                <ArrowLeft size={14} /> Back to sign in
              </Link>
            </div>
          ) : (
            <>
              <Link href="/login" className="text-[13px] font-medium hover:underline flex items-center gap-1.5 mb-5" style={{ color: 'var(--text-dark-secondary)' }}>
                <ArrowLeft size={14} /> Back to sign in
              </Link>
              <h2 className="text-[20px] md:text-[22px] font-bold mb-1" style={{ color: 'var(--text-dark)', fontFamily: 'Satoshi' }}>Reset password</h2>
              <p className="text-[13px] md:text-[14px] mb-6" style={{ color: 'var(--text-dark-secondary)' }}>Enter your email and we&apos;ll send a reset link.</p>
              {error && (
              ...
              <form onSubmit={handleReset} className="space-y-4">
                ...
                <button type="submit" disabled={loading || !email}
                  ...
                  style={{ background: 'var(--accent)', color: '#0b0e14', opacity: loading || !email ? 0.6 : 1 }}>
