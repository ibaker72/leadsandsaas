...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const agentOrgId = getStringField(agent, 'org_id');
    if (!agentOrgId) return NextResponse.json({ error: 'No org' }, { status: 404 });
    const { data: org } = await db.from('organizations').select('*').eq('id', agentOrgId).single();
    ...
    const leadPhone = getStringField(lead, 'phone_e164');
    const leadSmsConsent = getBooleanField(lead, 'sms_consent');
    ...
    if (!convId) { const ch = leadPhone && leadSmsConsent ? 'sms' : 'email'; const { data: nc } = await db.from('conversations').insert({ org_id: orgId, lead_id, agent_id, channel: ch }).select('id').single(); convId = nc?.id; }
