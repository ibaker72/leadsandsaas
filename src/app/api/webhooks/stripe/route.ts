...
const LIMITS: Record<ValidPlan, Record<string, number>> = {
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (event.type === 'customer.subscription.created' || event.type === 'customer.subscription.updated') {
      ...
      const rawPlan = lookupKey.split('_')[0];
      const planId: ValidPlan = isValidPlan(rawPlan) ? rawPlan : 'starter';
      await db.from('organizations').update({ stripe_subscription_id: sub.id, plan: planId, limits: LIMITS[planId] }).eq('stripe_customer_id', sub.customer as string);
  ...
  } catch (e) { console.error('Stripe webhook error:', e); return NextResponse.json({ error: 'Failed' }, { status: 500 }); }
}
